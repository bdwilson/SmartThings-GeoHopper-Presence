...
preferences {
  section ("Allow external service to control these things...") {
    input "presence", "capability.presenceSensor", multiple: true, required:
true
  }
}
...
def listLocations() {
    def resp = []
    presence.each {
      log.debug "RECEIVED: ${it.displayName}, attribute ${it.name}, ID:
${it.id}"
...
private void update (devices) {
   ...
   log.debug "update, request: params: ${params}, data, ${data}, devices:
$devices.id, $devices.displayName"
